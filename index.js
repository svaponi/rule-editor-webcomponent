...
var FormFieldConditionTypes = ['single_condition', 'multiple_condition'];
var FormFieldMultipleConditionOperators = ['and', 'or'];
var FormFieldSingleConditionOperators = [
    'eval',
    'regex',
    'equal',
    'equal_ignore_case',
...
function isCondition(conditionOrBool) {
    return typeof conditionOrBool !== 'boolean';
}
function isSingleCondition(condition) {
    return isCondition(condition) && condition.type === 'single_condition';
}
function isMultipleCondition(condition) {
    return isCondition(condition) && condition.type === 'multiple_condition';
}
function isValidSingleCondition(condition) {
    return (condition.type === 'single_condition' &&
        condition.operator &&
        FormFieldSingleConditionOperators.includes(condition.operator) &&
        condition.field_name &&
        condition.field_name.trim() !== '');
}
function isValidMultipleCondition(condition) {
    return (condition.type === 'multiple_condition' &&
        condition.operator &&
...
function ConditionOrBoolComponentV2(_a) {
    ...
    if (thisCondition === undefined || thisCondition === null) {
        return React.createElement(NewConditionComponent, { onChange: onChange });
    }
    if (!isCondition(thisCondition)) {
        return React.createElement(BooleanConditionComponent, { condition: thisCondition, editing: editing, onChange: onChange });
    }
    if (isSingleCondition(thisCondition)) {
        return React.createElement(SingleConditionComponent, { condition: thisCondition, editing: editing, onChange: onChange });
    }
    if (isMultipleCondition(thisCondition)) {
        return React.createElement(MultipleConditionComponent, { condition: thisCondition, editing: editing, onChange: onChange });
    }
    return (React.createElement("div", { className: "containerized condition-container unknown" },
        React.createElement("span", { className: 'json-container' }, JSON.stringify(thisCondition))));
}
function BooleanConditionComponent(_a) {
    var condition = _a.condition, onChange = _a.onChange, propsEditing = _a.editing;
    var _b = react.exports.useState(condition), thisBooleanCondition = _b[0], setThisBooleanCondition = _b[1];
    ...
    return (React.createElement("div", { className: "containerized condition-container boolean-condition" }, editing ? (React.createElement(React.Fragment, null,
        React.createElement("span", { className: 'form-container' },
            React.createElement("input", { type: 'checkbox', checked: thisBooleanCondition, onChange: function (e) { return setThisBooleanCondition(e.target.checked); } })),
        ...
        React.createElement("span", { className: 'json-container' }, JSON.stringify(thisBooleanCondition)),
...
var newMultiple = { type: 'multiple_condition', operator: undefined, value: [] };
function NewConditionComponent(_a) {
    var onChange = _a.onChange;
    var _b = react.exports.useState(FormFieldConditionTypes[0]), type = _b[0], setType = _b[1];
    function handleConfirm() {
        onChange(type === 'multiple_condition' ? newMultiple : newSingle);
    }
    return (React.createElement("div", { className: "containerized condition-container new-condition" },
...
function SingleConditionComponent(_a) {
    ...
    function setValue(value) {
        ...
        setThisSimpleCondition(function (prev) { return (__assign(__assign({}, prev), { value: value })); });
    }
    function handleChange(event) {
        console.log('handleChange', event.target.name, event.target.value);
        setThisSimpleCondition(function (prev) {
            var _a;
            return (__assign(__assign({}, prev), (_a = {}, _a[event.target.name] = event.target.value, _a)));
        });
    }
    function handleCheck(event) {
        ...
        setThisSimpleCondition(function (prev) {
            var _a;
            return (__assign(__assign({}, prev), (_a = {}, _a[event.target.name] = event.target.checked, _a)));
        });
    }
    var Type = React.createElement("input", { type: 'text', name: 'type', value: thisSimpleCondition.type, disabled: true });
    var FieldName = (React.createElement("input", { type: 'text', name: 'field_name', value: thisSimpleCondition.field_name, onChange: handleChange }));
    var OperatorOptions = FormFieldSingleConditionOperators.map(function (v) { return React.createElement("option", { value: v }, v); });
    ...
    return (React.createElement("div", { className: "containerized condition-container ".concat(isValidClass, " single-condition") }, editing ? (React.createElement(React.Fragment, null,
        React.createElement("span", { className: "form-container" },
            "type:",
            Type,
            " field_name:",
            ...
            Reverse),
        React.createElement("button", { onClick: function () { return handleCancel(); } }, "cancel"),
        React.createElement("button", { disabled: !isValid, onClick: function () { return handleSave(); } }, "save"))) : (React.createElement(React.Fragment, null,
        React.createElement("span", { className: 'json-container' }, JSON.stringify(thisSimpleCondition)),
        React.createElement("button", { onClick: function () { return setEditing(true); } }, "edit"),
        React.createElement("button", { onClick: function () { return handleDelete(); } }, "delete")))));
}
function MultipleConditionComponent(_a) {
    var onChange = _a.onChange, condition = _a.condition, propsEditing = _a.editing;
    var _b = react.exports.useState(condition), thisMultipleCondition = _b[0], setThisMultipleCondition = _b[1];
    var _c = react.exports.useState(false), adding = _c[0], setAdding = _c[1];
    var _d = react.exports.useState(propsEditing), editing = _d[0], setEditing = _d[1];
    ...
    function handleChangeChildCondition(newCondition, oldCondition) {
        ...
        if (newCondition === null && oldCondition === null) ;
            ...
            onChange(__assign(__assign({}, thisMultipleCondition), { value: __spreadArray([], thisMultipleCondition.value.filter(function (c) { return c !== oldCondition; }), true) }));
        }
        else if (!isCondition(newCondition)) {
            throw Error("unsupported new condition ".concat(newCondition));
        }
        else if (isCondition(newCondition)) {
            var value = __spreadArray([], thisMultipleCondition.value, true);
            var indexToRemove = value.indexOf(oldCondition);
            if (indexToRemove >= 0) {
            ...
            onChange(__assign(__assign({}, thisMultipleCondition), { value: value }));
        }
        else {
            throw Error("unsupported new condition ".concat(newCondition));
        }
    ...
    function handleChange(event) {
        console.log('handleChange', event.target.name, event.target.value);
        setThisMultipleCondition(function (prev) {
            var _a;
            return (__assign(__assign({}, prev), (_a = {}, _a[event.target.name] = event.target.value, _a)));
        });
    }
    var Type = React.createElement("input", { type: 'text', name: 'type', value: thisMultipleCondition.type, disabled: true });
    var OperatorOptions = FormFieldMultipleConditionOperators.map(function (v) { return React.createElement("option", { value: v }, v); });
    var Operator = (React.createElement("select", { name: 'operator', value: thisMultipleCondition.operator, onChange: handleChange },
        React.createElement("option", null),
        OperatorOptions));
    var children = thisMultipleCondition.value, conditionWithoutChildren = __rest(thisMultipleCondition, ["value"]);
    var isValid = isValidMultipleCondition(thisMultipleCondition);
    var isValidClass = isValid ? 'valid' : 'invalid';
    return (React.createElement("div", { className: "containerized condition-container ".concat(isValidClass, " multiple-condition ").concat(thisMultipleCondition.operator, "-condition") },
        editing ? (React.createElement(React.Fragment, null,
            ...
            React.createElement("button", { onClick: function () { return handleCancel(); } }, "cancel"),
            React.createElement("button", { disabled: !isValid, onClick: function () { return handleSave(); } }, "save"))) : (React.createElement(React.Fragment, null,
            React.createElement("span", { className: 'json-container' }, JSON.stringify(conditionWithoutChildren)),
            React.createElement("button", { disabled: adding, onClick: function () { return setAdding(true); } }, '+'),
            React.createElement("button", { disabled: editing, onClick: function () { return setEditing(true); } }, "edit"),
            React.createElement("button", { onClick: function () { return handleDelete(); } }, "delete"))), children === null || children === void 0 ? void 0 :
        children.map(function (child, index) { return (React.createElement("div", { key: JSON.stringify(child) },
            index > 0 && React.createElement("pre", null, condition.operator),
            React.createElement(ConditionOrBoolComponentV2, { condition: child, onChange: function (updatedCondition) { return handleChangeChildCondition(updatedCondition, child); } }))); }),
        adding && (React.createElement(React.Fragment, null,
            (children === null || children === void 0 ? void 0 : children.length) > 0 && React.createElement("pre", null, condition.operator),
            React.createElement(ConditionOrBoolComponentV2, { editing: true, condition: null, onChange: function (newCondition) { return handleChangeChildCondition(newCondition, null); } })))));
}

function isMessage(validationOrMessage) {
    return typeof validationOrMessage === 'string';
}
function isValidation(validationOrMessage) {
    return !isMessage(validationOrMessage);
}

function ValidationsComponentV2(_a) {
    var validations = _a.validations, onChange = _a.onChange;
    var _b = react.exports.useState(false), adding = _b[0], setAdding = _b[1];
    function handleChangeValidation(newValidation, oldValidation) {
        ...
        if (newValidation === null && oldValidation === null) ;
            ...
            onChange(__spreadArray([], validations.filter(function (c) { return c !== oldValidation; }), true));
        }
        else if (newValidation !== null) {
            var triggerCopy = __spreadArray([], validations, true);
            var indexToRemove = triggerCopy.indexOf(oldValidation);
            if (indexToRemove >= 0) {
            ...
            throw Error("unsupported new validation ".concat(newValidation));
        }
        setAdding(false);
    }
    return (React.createElement(React.Fragment, null,
        validations.map(function (validation) { return (React.createElement("div", { key: JSON.stringify(validation) },
            React.createElement(ValidationOrMessageComponentV2, { validation: validation, onChange: function (updated) { return handleChangeValidation(updated, validation); } }))); }),
        adding && (React.createElement(ValidationOrMessageComponentV2, { validation: { message: '', condition: null }, onChange: function (updated) { return handleChangeValidation(updated, null); } })),
        React.createElement("button", { disabled: adding, onClick: function () { return setAdding(true); } }, '+')));
}
function ValidationOrMessageComponentV2(_a) {
    var validation = _a.validation, onChange = _a.onChange;
    return isValidation(validation) ? (React.createElement(ValidationComponentV2, { validation: validation, onChange: onChange })) : (React.createElement(MessageComponent, { message: validation, onChange: onChange }));
}
function ValidationComponentV2(_a) {
    var validation = _a.validation, onChange = _a.onChange;
    function setCondition(condition) {
        onChange(__assign(__assign({}, validation), { condition: condition }));
    }
    function setMessage(message) {
        onChange(__assign(__assign({}, validation), { message: message }));
    }
    function handleDelete() {
        onChange(null);
    }
    return (React.createElement("div", { className: "containerized validation-container" },
        React.createElement("p", { className: "title message-title" }, "message"),
        React.createElement(MessageComponent, { message: validation.message, onChange: setMessage }),
        React.createElement("p", { className: "title condition-title" }, "condition"),
        React.createElement(ConditionOrBoolComponentV2, { condition: validation.condition, onChange: setCondition }),
        React.createElement("button", { onClick: function () { return handleDelete(); } }, "delete validation")));
}
function MessageComponent(_a) {
    var message = _a.message, onChange = _a.onChange, propsEditing = _a.editing;
    var _b = react.exports.useState(message), thisMessage = _b[0], setThisMessage = _b[1];
    var _c = react.exports.useState(propsEditing !== null && propsEditing !== void 0 ? propsEditing : false), editing = _c[0], setEditing = _c[1];
    react.exports.useEffect(function () {
...
function TriggersComponentV2(_a) {
    var triggers = _a.triggers, onChange = _a.onChange;
    var _b = react.exports.useState(false), adding = _b[0], setAdding = _b[1];
    function handleChangeTrigger(newTrigger, oldTrigger) {
        ...
        if (newTrigger === null && oldTrigger === null) ;
            ...
            onChange(__spreadArray([], triggers.filter(function (c) { return c !== oldTrigger; }), true));
        }
        else if (newTrigger !== null) {
            var triggerCopy = __spreadArray([], triggers, true);
            var indexToRemove = triggerCopy.indexOf(oldTrigger);
            if (indexToRemove >= 0) {
            ...
            throw Error("unsupported new trigger ".concat(newTrigger));
        }
        setAdding(false);
    }
    return (React.createElement(React.Fragment, null,
        triggers.map(function (trigger) { return (React.createElement("div", { key: JSON.stringify(trigger) },
            React.createElement(TriggerComponent, { trigger: trigger, onChange: function (updatedTrigger) { return handleChangeTrigger(updatedTrigger, trigger); } }))); }),
        adding && (React.createElement(TriggerComponent, { trigger: { action: { type: 'eval', expression: '' }, condition: null }, onChange: function (updatedTrigger) { return handleChangeTrigger(updatedTrigger, null); } })),
        React.createElement("button", { disabled: adding, onClick: function () { return setAdding(true); } }, '+')));
}
function TriggerComponent(_a) {
    var trigger = _a.trigger, onChange = _a.onChange;
    function setCondition(condition) {
        onChange(__assign(__assign({}, trigger), { condition: condition }));
    }
    function setAction(action) {
        onChange(__assign(__assign({}, trigger), { action: action }));
    }
    function handleDelete() {
        onChange(null);
    }
    return (React.createElement("div", { className: "containerized trigger-container" },
        React.createElement("p", { className: "title action-title" }, "action"),
        React.createElement(TriggerActionComponent, { action: trigger.action, onChange: setAction }),
        React.createElement("p", { className: "title condition-title" }, "condition"),
        React.createElement(ConditionOrBoolComponentV2, { condition: trigger.condition, onChange: setCondition }),
        React.createElement("button", { onClick: function () { return handleDelete(); } }, "delete trigger")));
}
...
function RuleEditor(_a) {
    ...
    var _b = react.exports.useState(), ruleId = _b[0], setRuleId = _b[1];
    var _c = react.exports.useState(), rule = _c[0], setRule = _c[1];
    var _d = react.exports.useState(), type = _d[0], setType = _d[1];
    react.exports.useEffect(function () {
        if (propsRuleId == undefined)
            return;
        else if (typeof propsRuleId === 'string')
            setRuleId(propsRuleId);
        else if (typeof propsRuleId === 'number')
            setRuleId("".concat(propsRuleId));
        else
            throw Error('invalid ruleId ' + propsRuleId);
    }, [propsRuleId]);
    react.exports.useEffect(function () {
        if (propsRule == undefined)
            return;
        else if (typeof propsRule === 'string')
            setRule(JSON.parse(propsRule));
        else if (typeof propsRule === 'object')
            setRule(propsRule);
        else if (typeof propsRule === 'boolean')
            setRule(propsRule);
        else
            throw Error('invalid rule ' + propsRule);
    }, [propsRule]);
    react.exports.useEffect(function () {
        if (propsType == undefined)
            return;
        else if (propsType === 'hidden')
            setType(propsType);
        else if (propsType === 'disabled')
            setType(propsType);
        else if (propsType === 'required')
            setType(propsType);
        else if (propsType === 'validations')
            setType(propsType);
        else if (propsType === 'triggers')
            setType(propsType);
        else
    ...
    return (React.createElement("div", { className: "rule-editor", id: 'rule-editor-' + ruleId },
        ...
        rule === undefined ? (React.createElement("span", null,
...
function RuleEditorInternal(_a) {
    ...
    if (type === 'hidden' || type === 'disabled') {
        var condition = rule;
        return React.createElement(ConditionOrBoolComponentV2, { condition: condition, onChange: setRule });
    }
    if (type === 'required') {
        var validation = rule;
        return React.createElement(ValidationOrMessageComponentV2, { validation: validation, onChange: setRule });
    }
    if (type === 'validations') {
        var validations = rule;
        return React.createElement(ValidationsComponentV2, { validations: validations, onChange: setRule });
    }
    if (type === 'triggers') {
        var triggers = rule;
        return React.createElement(TriggersComponentV2, { triggers: triggers, onChange: setRule });
    }
